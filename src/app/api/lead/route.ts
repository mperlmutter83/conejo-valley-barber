...
const FROM = 'Conejo Valley Barber <info@yesidoinc.com>';
...
async function notifyWorkApplicant(opts: {
  ...
  const html = `
    <div style="font-family:system-ui,-apple-system,sans-serif;font-size:15px;color:#111;">
      <h2 style="margin:0 0 12px;">New work applicant — conejovalleybarber.com</h2>
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        subject: `New work applicant — ${opts.name} (conejovalleybarber.com)`,
        html,
      }),
    });
    if (!res.ok) {
